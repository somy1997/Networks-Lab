...
int main(int argc, char **argv)
{
	...
	int sockfd,n[WINDOWSIZE],n1,timeouts;
	unsigned short portno;
	struct sockaddr_in serveraddr;
	socklen_t serverlen = sizeof(serveraddr);
	char *hostname, *chunk;
	char buf[WINDOWSIZE*BUFSIZE], ack[BUFSIZE];
	unsigned int seqno = 0, oldackno, ackno, discard, outstanding;
	...
	n[0] = encode(buf, seqno, strlen(chunk));
	
	timeouts = 0;
	while(1)
	{
		n1 = sendto(sockfd , buf, n[0], 0, (struct sockaddr *)&serveraddr, serverlen);
		if (n1 < 0)
		{ 
	...
	while(!feof(fin) || ackno < seqno)
	{
		...
		while(!feof(fin) && outstanding < WINDOWSIZE)
		{
			seqno++;
			bzero(buf + outstanding*BUFSIZE, BUFSIZE);
			chunk = buf + outstanding*BUFSIZE + HEADSIZE;
			n[outstanding] = fread(chunk, sizeof(char), CHUNKSIZE, fin);
			MD5_Update (&mdContext, chunk, n[outstanding]);
			n[outstanding] = encode(buf + outstanding*BUFSIZE, seqno, n[outstanding]);
			n1 = sendto(sockfd , buf + outstanding*BUFSIZE, n[outstanding], 0, (struct sockaddr *)&serveraddr, serverlen);
			if (n1 < 0)
			{ 
				cerr<<"ERROR writing to socket\n";
				exit(0);
		...
		while(1)
		{
			...
			if(n1<0 && errno == EWOULDBLOCK)
			{
				...
				for(unsigned int i=0; i<outstanding; i++)
				{
					...
					//cout<<*((int *)temp)<<endl;
					//cout<<n[i]<<endl;
					n1 = sendto(sockfd , buf + i*BUFSIZE, n[i], 0, (struct sockaddr *)&serveraddr, serverlen);
					if (n1 < 0)
					{ 
						cerr<<"ERROR writing to socket\n";
						exit(0);
